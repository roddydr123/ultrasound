...
def test_extract_Ls():
    video_paths = [f"{PATH_TO_FILES}/vid50.txt",
            f"{PATH_TO_FILES}/vid51.txt",
            f"{PATH_TO_FILES}/vid56.txt",
            f"{PATH_TO_FILES}/vid57.txt"]
    ...
    with open(f'{PATH_TO_FILES}/saved_L_dict.pkl', 'rb') as f:
        loaded_dict = pickle.load(f)

    with open(f'{PATH_TO_FILES}/saved_lengths.pkl', 'rb') as f:
        loaded_lengths = pickle.load(f)

    with open(f'{PATH_TO_FILES}/saved_diameters.pkl', 'rb') as f:
...
def test_calc_R_and_extract_Ls(video_numbs, Resints):
    video_paths = [f"{PATH_TO_FILES}/vid{video_numbs[0]}.txt",
            f"{PATH_TO_FILES}/vid{video_numbs[1]}.txt",
            f"{PATH_TO_FILES}/vid{video_numbs[2]}.txt",
            f"{PATH_TO_FILES}/vid{video_numbs[3]}.txt"]
...
def test_slice_thickness():
    ...
    viddata = {"PATH_TO_VIDEO": PATH_TO_FILES + "/vid50.mp4", "filenumber": "50",
               "PATH_TO_DETAILS": PATH_TO_FILES + "/details.txt"}
    vid = Video(viddata)
    ...
    test_data = np.load(f"{PATH_TO_FILES}/vid50_test.npy")
...
def test_calc_R_EPP_data():
    ...
    NHS_results, all_lengths, diameters = read_from_excel(PATH_TO_FILES + "/res_ints_only.csv")
    ST_ = []
    ...
    test_data = np.load(f"{PATH_TO_FILES}/EPP_calc_R_results.npy")
